...
typedef struct  {
	...
	int flag;        //操作类型,用"0"表示以读操作方式开文件的,用"1"表示写操作方式打开文件的
	pointer read;    //读文件的位置,文件刚打开时dnum 为文件起始盘
...
int sopen(char *name)     //在已打开文件表中查找文件name
{
	int i;
	i = 0;
	//依次查找已打开文件表
...
int dopen(char *name)     //在已打开文件表中删除文件name
{
	int i;
	i = sopen(name);
	if (i == -1)
...
iopen(content *x)       //在已打开文件表中插入文件x->name
{
	int i;
	i = sopen(x->name);
	if (i != -1)
...
int allocate()     //分配一个磁盘块,返回块号
{
	...
	fread(buffer1, 64L, 1, fc);//将FAT表中的第二个盘块读入模拟缓冲buffer1中
	for (i = 0; i<63; i++)
		if (buffer1[i] == 0)//FAT中的第i项为0,分配第i+64块磁盘块,修改FAT表,并且写回磁盘
		{
		buffer1[i] = 255;
		fseek(fc, -64L, SEEK_CUR);
		fwrite(buffer1, 64L, 1, fc);//返回磁盘号
		return(i + 64);
		}
	printf("已经没有磁盘空间\n");
	return(false);
...
while (t<length&&buffer1[openfile.file[i].read.bnum] != '#')
{
	...
	if (openfile.file[i].read.bnum >= 64)       //一块读完,读取下一个盘块
	{
		...
		fread(buffer1, 64, 1, fc);
		openfile.file[i].read.bnum = buffer1[openfile.file[i].read.dnum % 64];//修改读指针
...
while (t<length)
{
	...
	{
		fseek(fc, openfile.file[i].write.dnum * 64L, SEEK_SET);
		fread(buffer1, 64, 1, fc);//一块写完,写回磁盘
...
{//pna=从中分离出"/"后一个目录(或文件名)
	...
	j = 0;
	if (last == 1 && flag != 8)//查找名字pna、类型名pna的文件目录项
...
for (t = 0; t<8; t++)
{
	if (buffer2[t].name[0] == dname[0] && buffer2[t].name[1] == dname[1] &&
		buffer2[t].name[2] == dname[2] && buffer2[t].type[0] == tname[0] && buffer2[t].type[1] == tname[1])
	{//找到名字的文件,建立失败
		printf("文件已经存在,不能建立\n");
		return(false);
	}
...
if ((dd = allocate()) == false)//分配给建立目录的磁盘盘块
{
	printf("建立文件失败\n");
	return(false);
}
...
if (attribute == 1)
{
	while (d != '\xff')//寻找文件末尾
	{
		...
		last = d;
		d = buffer1[d % 64];//读出dnum块下一块内容献给dnum
	}//while
...
for (; k<3; k++)
	dname[k] = ' ';
if ((d = search(pathname,  & dn, &bn)) == false)//找到目录路径
{
	printf("目录不存在,不能建立\n");
	return(false);
}
...
//确认该目录不存在的同时查找空目录项
fseek(fc, d * 64L, SEEK_SET);
fread(buffer2, 64L, 1, fc); //读出盘块的内容
for (t = 0; t<8; t++)
{
	if (buffer2[t].name[0] == dname[0] && buffer2[t].name[1] == dname[1] &&
...
void main()
{
	...
	for (i = 3; i<64; i++)
		buffer1[i] = 0;
	buffer1[13] = buffer1[49] = 254;//假定模拟磁盘中有两个坏盘块:第13块和第49块
	...
	fwrite(buffer1, 64L, 1, fc);
	for (i = 0; i<8; i++)
		buffer2[i].name[0] = '$';//若目录项第一个字符为"S"表示该目录项为空
	fwrite(buffer2, 64L, 1, fc);
	//初始化已打开文件表
	while (1)
	{
