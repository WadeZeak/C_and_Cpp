================================================================================
    MICROSOFT 基础类库 : 劫持测试 项目概述
===============================================================================

应用程序向导已为您创建了此 劫持测试 应用程序。此应用程序不仅演示 Microsoft 基础类的基本使用方法，还可作为您编写应用程序的起点。

本文件概要介绍组成 劫持测试 应用程序的每个文件的内容。
...
劫持测试.h
    这是应用程序的主头文件。
    其中包括其他项目特定的标头（包括 Resource.h），并声明 C劫持测试App 应用程序类。

劫持测试.cpp
    这是包含应用程序类 C劫持测试App 的主应用程序源文件。

劫持测试.rc
    这是程序使用的所有 Microsoft Windows 资源的列表。它包括 RES 子目录中存储的图标、位图和光标。此文件可以直接在 Microsoft Visual C++ 中进行编辑。项目资源包含在 2052 中。

res\劫持测试.ico
    这是用作应用程序图标的图标文件。此图标包括在主资源文件 劫持测试.rc 中。
...
劫持测试Dlg.h、劫持测试Dlg.cpp - 对话框
    这些文件包含 C劫持测试Dlg 类。此类定义应用程序的主对话框的行为。对话框模板包含在 劫持测试.rc 中，该文件可以在 Microsoft Visual C++ 中编辑。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 劫持测试.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
